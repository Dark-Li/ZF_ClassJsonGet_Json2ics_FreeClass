...
class Client:
    ...
    def get_block_courses(self, year: int, term: int, block: int, params: dict = None):
        """获取板块课选课列表"""
        try:
            # 获取head_data
            url_head = urljoin(
            ...
            # 如果提供了额外参数，添加到URL
            if params:
                url_head += "&" + "&".join([f"{k}={v}" for k, v in params.items()])
...
if __name__ == "__main__":
    ...
    """ 获取学业情况 """
    # result1 = lgn.get_academia()

    """ 获取GPA """
    # result = lgn.get_gpa()

    """ 获取课程表 """
    result = lgn.get_schedule(2025, 1)
